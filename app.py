...
def match_food_with_ai_fallback(food_input, available_foods):
    """Claude APIを使った高度なマッチング（フォールバック用）"""
    if not ANTHROPIC_AVAILABLE:
        print("anthropicモジュールが利用できません")
        return None
...
def calculate_nutrition():
    ...
    try:
        ...
        for item in parsed_items:
            # AIは使わずに、あいまい検索を使用（高速＆安定）
            matched_food_name = fuzzy_match_food(item['food_name'], FOOD_DATABASE, use_ai=False)
